...
class RoleZero(Role):
    ...
    async def _think(self) -> bool:
        ...
        for attempt in range(max_retries):
            async with ThoughtReporter(enable_llm_stream=True) as reporter:
                ...
                # 如果是重试，在请求中添加强调必须返回响应的提示
                if attempt > 0:
                    retry_prompt = prompt + f"\n\n[重要提示：你之前返回了空响应。请务必返回有效的命令JSON数组，不能返回空内容。必须包含至少一个命令或使用end命令。]"
                    retry_req = self.llm.format_msg(memory + [UserMessage(content=retry_prompt)])
                    self.command_rsp = await self.llm_cached_aask(req=retry_req, system_msgs=[system_prompt], state_data=state_data)
                else:
                    self.command_rsp = await self.llm_cached_aask(req=req, system_msgs=[system_prompt], state_data=state_data)
            
            # 检查是否为空响应或响应过短（可能是无效响应）
            response_length = len(self.command_rsp.strip()) if self.command_rsp else 0
            if response_length > 10:  # 至少10个字符才认为有效
                # 有响应，跳出重试循环
                break
            else:
                # 空响应或响应过短，记录并重试
                if attempt < max_retries - 1:
                    logger.warning(f"收到空响应或响应过短 (长度: {response_length})，第 {attempt + 1}/{max_retries} 次重试中... (等待 {retry_delay} 秒)")
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2  # 指数退避
                else:
                    logger.error(f"收到空响应或响应过短 (长度: {response_length})，已重试 {max_retries} 次，放弃重试")
    ...
    async def _act(self) -> Message:
        ...
        if not ok:
            # Handle different return types: commands can be a list (empty response) or a string (error message)
            if isinstance(commands, list):
                error_msg = "LLM returned empty response. No commands were generated."
            else:
                error_msg = str(commands) if commands else "LLM returned empty response. No commands were generated."
