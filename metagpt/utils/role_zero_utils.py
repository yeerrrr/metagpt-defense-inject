...
from metagpt.injector.Tester2 import parse_commands_fault_injector
@parse_commands_fault_injector
async def parse_commands(command_rsp: str, llm, exclusive_tool_commands: list[str]) -> Tuple[list[dict], bool]:
    ...
    """
    # Quick defensive check: if the raw response is empty or only whitespace,
    # return immediately and record the event. This prevents json.loads on
    # empty strings which raises JSONDecodeError: Expecting value.
    if not command_rsp or not command_rsp.strip():
        logger.warning("😭😭parse_commands received empty command_rsp (len=0 or whitespace). Returning early.")
        # Return empty command list, failure flag, and echo original response
        return [], False, command_rsp
